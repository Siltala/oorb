...
CONTAINS
  ...
  SUBROUTINE massEstimation_MCMC(storb_arr, orb_arr, &
   ...
   ALLOCATE(additional_perturbers(nperturber,8), &
    ...
    lambda_arr(1,:) = 2.4**2/(6.0d0*nstorb+nperturber) ! Use this with AM; default scale factor for all
    cov_matrix = 0
    A_arr2 = 0.0_bp
    mean_arr = 0.0_bp
    nsigma_mass = 1
    ...
    DO
       ...
       IF (accept .EQV. .TRUE. .AND. chi2_compared .EQV. .FALSE.) THEN
          ...
          IF (iorb == 0 .OR. MOD(iorb,500) == 0) THEN ! Writes the residuals every 500 accepted proposals.
             WRITE(53, *) "#", iorb
             CALL writeMeanResids(storb_arr,orb_arr)
          END IF

          ! Outlier detection happens here. We also need to get
          ! updated chi2 values after.
          IF (iorb == 250 .OR. iorb == 2500 .OR. iorb == 5000 ) THEN
       ...
       IF (accept) THEN
          ...
          IF ((.NOT. first) .and. (iorb > 1)) THEN ! This is how many repetitions there were. can't be printed until the next 
            ! proposal gets accepted because we don't know the amount until then..
             WRITE(52, "(1(I5))", advance="yes") NINT(accepted_solutions(8*nstorb+3,iorb-1))
          END IF
          WRITE(52, "(1(I5,1X),1(I10,1X), 2(I5,1X), 1(F8.2,1X))", advance="NO") iorb, itrial, nperturber, nstorb-nperturber, mjd_tt
          DO i=1,nstorb
             WRITE(52,"(A5,1X,1(3(F19.15,1X),3(F19.15,1X),3(E12.5,1X)))",advance="NO") &
                  getID(storb_arr(i)), &
                  accepted_solutions(8*(i-1)+1:8*i,iorb), rchi2_arr(i)!, & ! accepted: 6 fitted orbital elements, fitted mass, chi2
          END DO
          WRITE(52, "(3(F19.2,1X))", advance="NO") SUM(chi2_arr), SUM(chi2_arr)/(SUM(nobs_arr)-6*nstorb-nperturber) &
               , EXP(-0.5*SUM(chi2_arr)/SUM(nobs_arr))
      END IF
       IF (info_verb >= 3) THEN
          WRITE(stderr, *) "-----------------------------------"
       END IF
       ...
       IF ((iorb - iorb_init) > 1 .AND. iorb .NE. norb/2) THEN
          ...
          IF ((adaptation == "aswam" .OR. adaptation == "gaswam") .AND. (iorb - iorb_init) > 19) THEN
             ...
             lambda_arr(1,:) = EXP(LOG(lambda_arr(1,1))+itrial**(-0.5_bp)*(a_r-0.237)) !"Optimal": 0.237
          ...
          IF (adaptation .NE. "none") THEN
             ...
             DO j=1,6*nstorb+nperturber
                A_arr2(j,j) = p2(j)
                A_arr2(j,j+1:6*nstorb+nperturber) = 0
             END DO
             !stop
          END IF
       END IF
    ...
    IF (last) THEN
       ...
       DO i=1,nperturber
          ...
          DO j=1,3
             ...
             IF (j == 1) THEN
                WRITE(52,"(A,1X,E13.6)") "# Nominal: ", &
                     estimated_masses(i,1)
                WRITE(52,"(A,2(1X,E13.6))") "# 1-sigma: ", bounds
             ELSE IF (j == 2) THEN
                WRITE(52,"(A,2(1X,E13.6))") "# 2-sigma: ", bounds
             ELSE IF (j == 3) THEN
                WRITE(52,"(A,2(1X,E13.6))") "# 3-sigma: ", bounds
             END IF
          END DO
       END DO
