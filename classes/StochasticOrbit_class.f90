...
!!$obsies_ccoords(2)%velocity = 0.0_bp
  ...
  SUBROUTINE writeMeanResids(this, orb_arr)
    IMPLICIT NONE
    ...
    DO i=1, SIZE(this)
       ...
       DO j=1, nobs_arr(i)/2
          ...
          IF (obs_masks(j,2) .EQV. .FALSE.) THEN
             WRITE(53, *) " ",k, this(i)%mean_residuals(j,2:3)/rad_asec, 0
          ELSE
             WRITE(53, *) " ",k, this(i)%mean_residuals(j,2:3)/rad_asec, 1
          END IF
          IF (j == nobs_arr(i)/2) THEN
             IF (i == SIZE(this)) THEN
               WRITE(53, *) "END" ! Means we're done printing residuals
             ELSE
               WRITE(53, *) "* * * *" !Signifies us moving to another object
             END IF
