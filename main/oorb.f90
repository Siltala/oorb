...
!!$           obj_vmag = mag
  ...
  CASE ("mass_estimation_mcmc")
     ...
     ! Output files for the mcmc.
     ! FIXME: use that function for figuring out proper unit numbers
     OPEN(unit=52, file=TRIM(out_fname), action='WRITE', iostat=err)
     OPEN(unit=53, file=TRIM(out_fname)//trim('.res'), action='WRITE', iostat=err)
     OPEN(unit=54, file=TRIM(out_fname)//trim('.cov'), action='WRITE', iostat=err)
     ...
     IF (mass == 0.0) THEN
        DO i = 1, nperturber
           IF (mass_arr_in(i) <= 0) THEN
              IF (HG_arr_in(i,1) < 99.0) THEN
                 ...
                 proposal_density_masses(i) = const*10**(-0.6*HG_arr_in(i, 1))
             ELSE ! This happens if the orbit doesn't include brightness info for some reason.
                 ...
                 proposal_density_masses(i) = 0.5e-11
             END IF
           ELSE
